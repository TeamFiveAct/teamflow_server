...
module.exports = (io) => {
  io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', async ({ user_id, workspace_id }) => {
      ...
      try {
        ...
        if (!chatRoom) {
          socket.emit(
            'error',
            '해당 workspace에 대한 채팅방이 존재하지 않습니다.'
          );
          return;
        }
      } catch (err) {
      ...
      console.log(
        `user_id=${user_id} 님이 workspace_id=${workspace_id} 채팅방에 입장`
      );
    ...
    socket.on(
      'sendMessage',
      async ({ user_id, workspace_id, content, content_type = 'text' }) => {
        const messageObj = {
          user_id,
          workspace_id,
          content,
        ...
        try {
          ...
          io.to(workspace_id.toString()).emit(
            'receiveMessage',
            messageWithUser
          );
        } catch (error) {
    ...
    socket.on('leaveRoom', ({ workspace_id }) => {
      ...
      console.log(
        `Socket ${socket.id} 님이 workspace_id=${workspace_id} 채팅방에서 퇴장`
      );
    });
